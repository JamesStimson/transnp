...
> Generation time density for a given individual

```{r}
plotGenerationTimeDensity(record[[1]], index=2)
```

> Infection-to-sample time density for a given individual

```{r}
plotSampleTimeDensity(record[[1]], index=2)
```
...
```{r}
getLikelyInfectors(record[[1]], 'c405')
```
